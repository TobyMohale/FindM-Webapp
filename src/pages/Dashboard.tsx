...
export default function Dashboard() {
  ...
  const handleSignUpNext = async (e: React.FormEvent) => {
    ...
    const cleanTagId = signupTagId.trim().toLowerCase();
    if (!cleanTagId) {
      setAuthMsg('Wristband Tag Code is required. Parents cannot register without a pre-printed physical wristband code.');
      return;
    }
    ...
    try {
      ...
      if (tagError) {
        setAuthMsg(`Verification error: ${tagError.message}`);
        setAuthLoading(false);
        return;
      }

      if (!tagData || tagData.length === 0) {
        setAuthMsg(`Invalid Code (${cleanTagId}). This wristband code has not been generated by the administrator. Parents cannot create their own codes.`);
        setAuthLoading(false);
        return;
      }

      const existingTag = tagData[0];
      if (existingTag.owner_id) {
        setAuthMsg(`This Wristband Code (${cleanTagId}) has already been claimed by another parent.`);
        setAuthLoading(false);
        return;
      }
      ...
      setAuthMsg(`Error validating wristband code: ${err.message || err}`);
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    if (!claimErr) {
      ...
      // Fallback if claim_tag RPC isn't deployed on an older instance or direct query is needed
      ...
      if (existingTagData && existingTagData.length > 0) {
        ...
        if (!updateError) {
          ...
          setAuthMsg(`Database Error updating tag: ${updateError.message || JSON.stringify(updateError)}.`);
          setAuthLoading(false);
          return;
        }
      } else {
  ...
  const handleClaimTag = async () => {
    ...
    try {
      ...
      if (!claimError) {
        ...
        const { data: tagData } = await supabase.from('tags').select('tag_id, owner_id').eq('tag_id', cleanTagId);
        if (!tagData || tagData.length === 0) {
          alert(`Invalid Code (${cleanTagId}). This wristband code has not been generated by the administrator.`);
          setSaving(false);
          return;
        }
        if (tagData[0].owner_id && tagData[0].owner_id !== user.id) {
          alert(`This Wristband Code (${cleanTagId}) has already been claimed by another parent.`);
          setSaving(false);
          return;
        }
        const { error: directErr } = await supabase.from('tags').update({
          owner_id: user.id,
          child_name: 'Emma',
          avatar: '👧',
          claimed_at: new Date().toISOString()
        ...
        if (directErr) {
          alert('Could not claim tag: ' + directErr.message);
        } else {
          setTagToClaim('');
          const { data: newTags } = await supabase.from('tags').select('*').eq('owner_id', user.id);
          if (newTags) {
            ...
            <div className="relative z-10 text-left">
              ...
              {signUpStep === 1 && (
                <form onSubmit={handleSignUpNext} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">6-Character Wristband Code (Required)</label>
                    <input 
                      type="text" 
                      ...
                      required
                      value={signupTagId}
                      onChange={e => setSignupTagId(e.target.value.toLowerCase())}
                  ...
                  <button 
                    type="submit"
                    disabled={!email || !childName || !parentPhone || !popiaConsent || !signupTagId}
